/**
 *  STM32F4 template project with LED, USART, Button, MEMS and USB FATFS
  ...
  while(1) {
    ...
    if (TM_DISCO_ButtonPressed()) {
      ...
      if(counter%500000 == 0) {
        ...
        printf("Axes X:%d, Y:%d, mainCounter:%d, usbCounter:%d\n\r",
               Axes_Data.X, Axes_Data.Y, mainCounter, usbCounter);
    ...
    if (1) {
      ...
      if (f_mount(&USB_Fs, "1:", 1) == FR_OK) {
        ...
        if (f_open(&USB_Fil, filename, FA_READ | FA_WRITE | FA_OPEN_ALWAYS) == FR_OK) {
          ...
          /* Put data */
          sprintf(writeBuffer, "File Counter: %u - %u\n", (uint)fileCounter, (uint)free);
          f_puts(writeBuffer, &USB_Fil);
          f_close(&USB_Fil);
        ...
        if (f_open(&USB_Fil, filename, FA_READ | FA_OPEN_ALWAYS) == FR_OK) {
          f_gets(readBuffer, 40, &USB_Fil);
          f_close(&USB_Fil);
